...
export function VLMSettings({
  ...
  // Track if form has been initialized to prevent re-resetting on every settings update
  const isInitializedRef = useRef(false);

  useEffect(() => {
    // Only reset form once during initial load
    if (!isInitializedRef.current && Object.keys(settings).length) {
  ...
  useEffect(() => {
    ...
    if (newProvider !== VLMProviderV2.agible) {
      form.setValue('vlmProvider', VLMProviderV2.agible);
    }
  }, [newProvider]);
    ...
    <>
      <Form {...form}>
        <form className={cn('space-y-8 px-[1px]', className)}>
          ...
          <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg border border-gray-200 dark:border-gray-700 mb-6 flex items-center gap-4">
            <div className="h-10 w-10 shrink-0 flex items-center justify-center bg-white dark:bg-gray-800 rounded-md border border-gray-200 dark:border-gray-700 p-1">
              <img src={logoBlack} className="w-full h-full object-contain dark:hidden" alt="Operator" />
              <img src={logoWhite} className="w-full h-full object-contain hidden dark:block" alt="Operator" />
            </div>
            <div>
              <p className="text-sm text-gray-900 dark:text-gray-100 font-medium">Model provided by <span className="font-bold">Agible</span></p>
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-xs font-semibold bg-black text-white dark:bg-white dark:text-black px-1.5 py-px rounded text-[10px] uppercase tracking-wider">Operator</span>
                <span className="text-xs text-gray-500 dark:text-gray-400">High-Capability Agent Model</span>
              </div>
            </div>
          </div>
...
export function ModelAvailabilityCheck({
  ...
  const handleCheckModel = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (isAvailable) {
        const successMessage = `Model is available and working correctly${
