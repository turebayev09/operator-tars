...
import { getExternalPkgs } from './scripts/getExternalPkgs.ts';
import {
  getModuleRoot,
...
async function cleanSources(
  ...
  try {
    if (platform === 'darwin' || platform === 'mas') {
      ...
      if (fs.existsSync(frameworkResourcePath)) {
    ...
    await Promise.all(
      Array.from(subDependencies.values()).map(async (subDependency) => {
        if (
    ...
    console.error('Error in cleanSources hook:', error);
    callback();
  }
}
  ...
  publishers: [
    {
      ...
      config: {
        repository: { owner: '', name: 'ui-tars-desktop' },
        draft: true,
        force: true,
