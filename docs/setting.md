...
**Desktop App** offers granular control over application behavior through its settings system. This document provides comprehensive guidance on configuration options, preset management, and operational best practices.
...
| ----------- | ---------------------- |
| **Type** | `string` |
| **Options** | - `Hugging Face for TARS-1.0`<br /> - `Hugging Face for TARS-1.5`<br /> - `VolcEngine Ark for Doubao-1.5-TARS`<br /> - `VolcEngine Ark for Doubao-1.5-thinking-vision-pro` |
| **Required** | `true` |
...
In the HuggingFace example in [TARS/README_deploy.md](https://github.com//TARS/blob/main/README_deploy.md#python-test-code), the VLM parameters are as follows:
...
In the VolcEngine(火山引擎) example in [Doubao-1.5-TARS](https://console.volcengine.com/ark/region:ark+cn-beijing/model/detail?Id=doubao-1-5-TARS), the VLM parameters are as follows:
...
> [!NOTE]
> Currently, there is no authentication designed for Report Storage Server. If you have any requirements, please submit an [issue](https://github.com//Desktop App/issues).
...
**UTIO** (_TARS Insights and Observation_) is a data collection mechanism for insights into **Desktop App** (_Introduced at [#60](https://github.com//Desktop App/pull/60)_). The design of UTIO is also related to sharing. The overall process is as follows:
