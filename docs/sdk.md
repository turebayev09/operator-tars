# @TARS/sdk Guide (Experimental)

[![NPM Downloads](https://img.shields.io/npm/d18m/@TARS/sdk)](https://www.npmjs.com/package/@TARS/sdk) [![codecov](https://codecov.io/gh//Desktop App/graph/badge.svg?component=ui_tars_sdk)](https://app.codecov.io/gh//Desktop App/components/ui_tars_sdk)

## Overview

`@TARS/sdk` is a powerful cross-platform(ANY device/platform) toolkit for building GUI automation agents.
...
Input your TARS Model Service Config(`baseURL`, `apiKey`, `model`), then you can control your computer with CLI.
...
sequenceDiagram
 ...
 participant model as TARS Model
...
Basic usage is largely derived from package `@TARS/sdk`, here's a basic example of using the SDK:
...
```ts
import { GUIAgent } from '@TARS/sdk';
import { NutJSOperator } from '@TARS/operator-nut-js';
...
Advanced sdk usage is largely derived from package `@TARS/sdk/core`, you can create custom operators by extending the base `Operator` class:
...
export class CustomOperator extends Operator {
 // Define the action spaces and description for TARS System Prompt splice
 static MANUAL = {
 ACTION_SPACES: [
...
- `MANUAL`: Define the action spaces and description for TARS Model understanding
 - `ACTION_SPACES`: Define the action spaces and description for TARS Model understanding
