...
| `visual-grounding` | Operation capabilities based on GUI Agent ([TARS](https://seed-tars.com/) / [Doubao 1.5 VL](https://www.volcengine.com/docs/82379/1554521)), without DOM-related tools. |
...
In Agent TARS's early preview version, we only supported Claude 3.7, but we still received numerous requests from the community for compatibility with various models. We initiated a Discussion about [Model Compatibility #377](https://github.com//Desktop App/discussions/377), but due to architecture limitations at that time, we couldn't quickly achieve good model compatibility.
...
For Agent TARS, this is a very valuable beginning. From the earliest Desktop App, GUI Agent was the only first-class citizen, but now, GUI Agent is no longer an isolated entity, playing its role in an integrated environment!
